...
SEXP forward(NumericVector diffs, NumericVector f, SEXP g) {
  ...
  // find f function
  integral_f.slice(0).fill(0.0); 
  for (int j=0; j<p; j++) {
    integral_f.slice(j+1) = integral_f.slice(j)+diffs(j)/2*(ff.slice(j)+ff.slice(j+1));
    for (int k=0; k<r; k++) integral_f.slice(j+1) += diffs(j)/(k+2)*gg.slice(j+k*p);
  }
